...
const microcode = {
	...
	arr_len: (cmd) => {
		if (cmd.expr.tag === "arr_lit") {
			return push(S, array_length(cmd.expr.elems));
		}

		if (cmd.expr.tag === "nam") {
			return push(S, array_length(lookup(cmd.expr.sym, E)));
		}

		if (cmd.expr.tag === "arr_in_stack") {
			return push(S, array_length(S.pop()));
		}

		push(C, { tag: "arr_len", expr: { tag: "arr_in_stack" } });
		push(C, cmd.expr);
	},
};
